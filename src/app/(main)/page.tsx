...
function addToHistory(entry: HistoryItem, userId?: string | null) {
  ...
  try {
    ...
    const updated: HistoryItem = {
      ...entry,
      watchedEpisodes: [...new Set([...prevWatched, entry.episode])],
      totalEpisodes:   entry.totalEpisodes ?? existing?.totalEpisodes,
      progress:        Math.max(entry.progress ?? 0, existing?.progress ?? 0),
    }
    localStorage.setItem(HISTORY_KEY,
      JSON.stringify([updated, ...prev.filter(e => e.malId !== entry.malId)].slice(0, 30))
    )
  } catch { }
}
...
export default function HomePage() {
  ...
  const currentHero   = heroList[heroIdx] ?? hero
...
function HistoryCard({ item, onRemove }: { item: HistoryItem; onRemove: (malId: string) => void }) {
  ...
  // Progress bar — tiga tier fallback:
  // 1. Seek position dari video player (0–100, normalise float 0–1 kalau perlu)
  // 2a. watchedEpisodes.length / totalEpisodes
  // 2b. lastEp / totalEpisodes
  // 3. Minimum 25%
...
function Skeleton() {
  return (
    <div className="flex flex-col gap-8 pb-16 px-4 animate-pulse">
      <div className="w-full bg-zinc-900" style={{ height: '58vw', maxHeight: '520px' }} />
      <div className="flex gap-2.5">
        {Array.from({ length: 5 }).map((_, i) => (
          <div key={i} className="shrink-0 w-44 aspect-video bg-zinc-900 rounded-xl" />
        ))}
      </div>
      {[0, 1].map(g => (
        <div key={g} className="flex gap-2.5">
          {Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="shrink-0 w-32 aspect-[2/3] bg-zinc-900 rounded-xl" />
          ))}
        </div>
      ))}
    </div>
